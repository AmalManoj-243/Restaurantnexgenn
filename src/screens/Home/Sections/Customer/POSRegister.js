  ...
  useEffect(() => {
    const loadRegistersAndSessions = async () => {
      ...
      try {
        ...
        // Filter registers to only show those with 'restaurant' in the name (case-insensitive)
        const restaurantRegs = Array.isArray(regs) ? regs.filter(r => r.name && r.name.toLowerCase().includes('restaurant')) : [];
        setRegisters(restaurantRegs);
        // Filter open sessions to only show those with 'restaurant' in the config name (case-insensitive)
        const restaurantSessions = Array.isArray(sessions) ? sessions.filter(s => s.config_id && s.config_id[1] && s.config_id[1].toLowerCase().includes('restaurant')) : [];
        setOpenSessions(restaurantSessions);
      } catch (err) {
    ...
    return (
      <View style={[styles.card, styles.sessionItemCard]}>
        <View style={styles.cardHeader}>
          <Image source={require('@assets/images/logo/logo.png')} style={styles.cardIcon} />
          <View style={{ flex: 1 }}>
            <Text style={styles.sessionTitle}>{item.config_id?.[1] || item.config_id?.[0] || 'Restaurant'}</Text>
      ...
      <View style={styles.centered}>
        ...
        {loading ? (
          ...
          <FlatList
            ...
            style={{ width: '100%' }}
            ListEmptyComponent={<Text style={styles.emptyText}>No open registers for Restaurant.</Text>}
          />
        )}
        ...
        <FlatList
          ...
          style={{ width: '100%' }}
          ListEmptyComponent={<Text style={styles.emptyText}>No available restaurant registers.</Text>}
        />
      </View>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  cardIcon: { width: 84, height: 84, marginRight: 16, borderRadius: 12, resizeMode: 'cover' },
